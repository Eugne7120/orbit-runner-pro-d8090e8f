...
function CodeGraph() {
  ...
  return (
    ...
    >
      ...
      {modules.map((m) => (
        <g key={m.i}>
          <rect x={m.x - 22} y={m.y - 10} width="44" height="20" rx="3" fill="none" stroke="oklch(0.78 0.14 232 / 0.9)" strokeWidth="0.5" />
...
function Packets() {
  return (
    <svg viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid slice" className="absolute inset-0 h-full w-full opacity-[0.045]">
      {Array.from({ length: 5 }).map((_, i) => {
        const y = 120 + i * 160;
        return (
          <g key={i}>
            <line x1={0} x2={1600} y1={y} y2={y} stroke="oklch(0.78 0.14 232 / 0.35)" strokeWidth="0.4" strokeDasharray="1 6" />
            {Array.from({ length: 4 }).map((_, j) => (
              <rect key={j} x={-40} y={y - 3} width="18" height="6" rx="1" fill="oklch(0.78 0.14 232)">
                <animate attributeName="x" from={-40} to={1640} dur={`${9 + (i * 1.4) + j}s`} begin={`${j * 1.2 + i * 0.4}s`} repeatCount="indefinite" />
                <animate attributeName="opacity" values="0;1;1;0" keyTimes="0;0.1;0.9;1" dur={`${9 + (i * 1.4) + j}s`} begin={`${j * 1.2 + i * 0.4}s`} repeatCount="indefinite" />
              </rect>
            ))}
          </g>
        );
...
function Credits() {
  return (
    <svg viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid slice" className="absolute inset-0 h-full w-full opacity-[0.05]">
...
function Blueprint() {
  return (
    <svg viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid slice" className="absolute inset-0 h-full w-full opacity-[0.04]">
      <defs>
        <pattern id="bp-grid" width="80" height="80" patternUnits="userSpaceOnUse">
          ...
          <path d="M 40 0 L 40 80 M 0 40 L 80 40" fill="none" stroke="oklch(0.78 0.14 232)" strokeWidth="0.2" />
        </pattern>
      </defs>
