...
import { MessageSquare, LayoutDashboard, CreditCard, ArrowRight, Activity, Globe, Clock } from "lucide-react";
...
const QUICK_ACTIONS = [
  { label: "Start chatting", desc: "Talk to 0RBIT AI", icon: MessageSquare, to: "/app/chat", accent: true },
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      ...
      <div className="mt-4 space-y-2">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="h-2 rounded-full bg-white/[0.05]" style={{ width: `${70 - i * 15}%` }} />
        ))}
      </div>
    </div>
  );
    ...
    <AppShell>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h1 className="text-2xl font-semibold text-foreground">Good to see you, {name.split(" ")[0]}.</h1>
          <p className="text-sm text-muted-foreground mt-1">The 0RBIT network is online and ready.</p>
        </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {QUICK_ACTIONS.map((a) => (
              <Link key={a.to} to={a.to}
                ...
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${a.accent ? "bg-signal/20" : "bg-white/[0.06]"}`}>
                  <a.icon className={`w-4 h-4 ${a.accent ? "text-signal" : "text-muted-foreground"}`} />
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3">Network Status</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {NETWORK_METRICS.map((m) => (
              <div key={m.label} className="rounded-xl border border-border bg-surface p-4 flex items-center gap-3">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
              ...
              <Placeholder title="Recent Activity" desc="Your latest conversations and requests will appear here." />
              <Placeholder title="Usage Summary" desc="Token consumption and model usage metrics." />
            </div>
            <div className="space-y-4">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Profile</h2>
              <UserProfile />
              <CreditsWidget />
            </div>
          </div>
