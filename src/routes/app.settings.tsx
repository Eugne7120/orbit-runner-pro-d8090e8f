...
function Toggle({ defaultOn = false }: { defaultOn?: boolean }) {
  ...
  return (
    <button onClick={() => setOn(!on)} className={cn("w-9 h-5 rounded-full transition-colors relative", on ? "bg-signal" : "bg-white/[0.12]")}>
      <span className={cn("absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform shadow-sm", on && "translate-x-4")} />
    </button>
  );
}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex gap-6">
            ...
            {SECTIONS.map((s) => (
              <button key={s.id} onClick={() => setSection(s.id)}
                className={cn("w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all text-left",
                  section === s.id ? "bg-white/[0.07] text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-white/[0.04]")}>
          ...
          <div className="sm:hidden flex overflow-x-auto gap-2 pb-2 mb-4 w-full">
            {SECTIONS.map((s) => (
              <button key={s.id} onClick={() => setSection(s.id)}
                className={cn("flex-shrink-0 px-3 py-1.5 rounded-lg text-xs transition-all",
                  section === s.id ? "bg-signal/15 text-signal border border-signal/25" : "bg-white/[0.04] text-muted-foreground border border-border")}>
                {s.label}
              </button>
            ))}
          </div>
          <motion.div key={section} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="flex-1 min-w-0">
            {section === "profile" && (
              <div className="space-y-4">
                <UserProfile />
                <div className="rounded-xl border border-border bg-surface p-5">
                  <h3 className="text-sm font-medium text-foreground mb-3">Display Name</h3>
                  <input defaultValue={user?.twitter?.name ?? user?.google?.name ?? ""}
                  ...
                  <button className="mt-3 px-4 py-2 rounded-lg bg-signal/15 border border-signal/25 text-signal hover:bg-signal/25 transition-colors text-sm font-medium">Save changes</button>
                </div>
              </div>
            )}
            {section === "appearance" && (
              <div className="rounded-xl border border-border bg-surface p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {["Dark", "System"].map((t) => (
                    <button key={t} className={cn("flex items-center gap-2 px-4 py-3 rounded-xl border text-sm transition-all",
                      t === "Dark" ? "border-signal/40 bg-signal/10 text-signal" : "border-border bg-white/[0.03] text-muted-foreground")}>
                      {t === "Dark" && <Check className="w-3.5 h-3.5" />}{t}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {section === "accounts" && (
              <div className="rounded-xl border border-border bg-surface p-5">
                ...
                <Row label="X (Twitter)" value={user?.twitter ? `@${user.twitter.username}` : "Not connected"} />
            ...
            {section === "security" && (
              <div className="rounded-xl border border-border bg-surface p-5">
                ...
                <div className="pt-3">
                  <button onClick={() => logout().then(() => navigate({ to: "/app/login" }))}
            ...
            {section === "notifications" && (
              <div className="rounded-xl border border-border bg-surface p-5">
                ...
                <Row label="Network updates" value="Major announcements" action={<Toggle defaultOn />} />
                <Row label="Credit alerts" value="Low balance warnings" action={<Toggle defaultOn />} />
